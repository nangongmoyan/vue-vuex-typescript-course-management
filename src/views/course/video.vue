...
/* eslint-disable */
import { aliyunOss } from '@/services/api'
      ...
      this.uploader = new window.AliyunUpload.Vod({
        ...
        onUploadstarted: async uploadInfo => {
          ...
          if (uploadInfo.isImage) {
            const { data } = await aliyunOss.aliyunImagUploadAddressAdnAuth()
            this.imageUrl = data.data.imageURL
            uploadAuthInfo = data.data
          } else {
            const { data } = await aliyunOss.aliyunVideoUploadAddressAdnAuth({
        ...
        onUploadEnd: async uploadInfo => {
          ...
          const { data } = await aliyunOss.transCodeVideo({
            lessonId: this.$route.query.lessonId,
            fileId: this.videoId,
            coverImageUrl: this.imageUrl,
            fileName: this.fileName
          ...
          setInterval(async () => {
             const { data } = await aliyunOss.getAliyunTransCodePercent(this.$route.query.lessonId)
