...
import { CreateOrUpdateCourseData } from '@/features/course'
  ...
  methods: {
    loadCourse () {
      courseApi.getCourseById(this.courseId).then(rlt => {
        const { activityCourseDTO: activityCourse, ...rest } = rlt.data

        const activityCourseDTO = this.course.activityCourseDTO
        if (activityCourse !== null) {
          Object.assign(activityCourseDTO, {
            endTime: activityCourse?.endTime ? moment(activityCourse.endTime).format('YYYY-MM-DD') : '',
            beginTime: activityCourse?.beginTime ? moment(activityCourse.beginTime).format('YYYY-MM-DD') : ''
          })
        }
        this.course = { ...this.course, ...rest, activityCourseDTO }
      })
    },

    handleSave () {
      courseApi.saveOrUpdateCourse(this.course).then(rlt => {
        if (rlt.code === '000000') {
